...
public final class RESTManager {
	...
	public static Pair<Integer, String> delete(String url, String username, String password, Map<String, String> headers, String contentType, String body) {
		CredentialsProvider credentials = new BasicCredentialsProvider();
		credentials.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials(username, password));

		logger.info("HTTP REQUEST: {} -> {} {} -> {}", url, username, ((password!=null)?password.length():"-"), contentType);
		if (headers != null) {
			logger.debug("Headers: ");
			headers.forEach((name, value) -> {
				logger.debug("{} -> {}", name, value);
		...
		try (CloseableHttpClient client = HttpClientBuilder.create().setDefaultCredentialsProvider(credentials).build()) {
			...
			if (response != null) {
				String returnBody = EntityUtils.toString(response.getEntity(), "UTF-8");
				logger.debug("HTTP DELETE Response Status Code: {}", response.getStatusLine().getStatusCode());
